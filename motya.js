(()=>{
  ...
  const FRAMES={idle:[0,0],sit:[1,0],sleep:[2,0],sad:[0,1],play:[1,1],walk1:[2,1],walk2:[0,2],walk3:[1,2],walk4:[2,2]};
  const WALK=['walk1','walk2','walk3','walk4'];
  const SHEET='assets/motya-sheet.webp?v=direct-2';
  const HOME={x:1120,y:748,w:230};
  const state={...HOME,pose:'idle',busy:false,sleeping:false,sleepTimer:0,walkTimer:0};

  function spriteMarkup(){return `<span class="motya-sprite" aria-hidden="true"><img class="motya-sheet-img" src="${SHEET}" alt="" draggable="false"></span>`}
  function setSprite(el,name){
    const [c,r]=FRAMES[name]||FRAMES.idle;
    const img=el.querySelector('.motya-sheet-img');
    if(img){img.style.left=(-c*100)+'%';img.style.top=(-r*100)+'%'}
  ...
  setPosition(HOME.x,HOME.y,HOME.w);setPose('idle');
  ...
  function walkTo(x,y,duration=1750){
    return new Promise(resolve=>{
      ...
      let frame=0;state.walkTimer=setInterval(()=>{frame=(frame+1)%4;setSprite(sprite,WALK[frame])},135);
      const ease=t=>t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;
      function step(now){const t=Math.min(1,(now-start)/duration),q=ease(t);setPosition(sx+(x-sx)*q,sy+(y-sy)*q,225);if(t<1)requestAnimationFrame(step);else{clearInterval(state.walkTimer);character.classList.remove('walking','facing-left');resolve()}}
      requestAnimationFrame(step);
    });
  }

  async function goSleep(){
    if(state.busy)return;state.busy=true;closeActions();const spot=restSpot();say('Мотя идёт отдыхать…',1600);
    await walkTo(spot.x,spot.walkY,1850);character.classList.add('pose-swap');await new Promise(r=>setTimeout(r,150));
    ...
    say('Тсс… Мотя уснула. Нажми на неё, чтобы разбудить.',3000);clearTimeout(state.sleepTimer);state.sleepTimer=setTimeout(()=>wakeUp(true),9000);
  }

  async function wakeUp(auto=false){
    if(!state.sleeping||state.busy)return;clearTimeout(state.sleepTimer);state.sleeping=false;state.busy=true;character.classList.remove('sleeping');
    const spot=restSpot();setPose('sit');setPosition(spot.x,spot.walkY,205);say(auto?'Мотя проснулась!':'Доброе утро, Мотя!',1600);
    await new Promise(r=>setTimeout(r,1100));await walkTo(HOME.x,HOME.y,1800);setPose('idle');setPosition(HOME.x,HOME.y,HOME.w);state.busy=false;
  }

  function play(){if(state.busy||state.sleeping)return;state.busy=true;closeActions();setPose('play');character.classList.add('happy-bounce');say('Ура! Поиграем ♡',1900);setTimeout(()=>{character.classList.remove('happy-bounce');setPose('idle');state.busy=false},2200)}
  function mood(){if(state.busy||state.sleeping)return;state.busy=true;closeActions();setPose('sad');character.classList.add('sad-mode');say('Я чуть-чуть скучаю. Погладишь меня?',2600);setTimeout(()=>{character.classList.remove('sad-mode');setPose('idle');state.busy=false},3000)}
  ...
  intro.addEventListener('click',e=>{if(e.target.closest('.motya-intro-close'))return closeIntro(false);const b=e.target.closest('[data-intro]');if(!b)return;if(b.dataset.intro==='care')return closeIntro(true);if(b.dataset.intro==='breed'){copy.hidden=true;introActions.hidden=true;introDog.classList.add('small');breed.hidden=false}else{breed.hidden=true;copy.hidden=false;introActions.hidden=false;introDog.classList.remove('small')}});
