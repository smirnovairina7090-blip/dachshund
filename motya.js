(()=>{
  ...
  async function goSleep(){
    ...
    setPose('sleep');setPosition(spot.sleepX,spot.sleepY,spot.sleepW);
  ...
  async function wakeUp(auto=false){
    ...
    const spot=restSpot();setPose('sit');setPosition(spot.sitX,spot.sitY,spot.sitW);
    say(auto?'Мотя проснулась!':'Доброе утро, Мотя!',1600);
    await new Promise(r=>setTimeout(r,1000));await walkTo(HOME.x,HOME.y,1800);
